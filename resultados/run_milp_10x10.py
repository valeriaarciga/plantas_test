...
P = Problema(10, 10, t=0.10)
esc = P.escenario(seed=0)
print(f"Instancia 10x10: nV={P.nV} nA={P.nA} cuota={list(P.quota)}", file=sys.stderr)
print(f"Nodos preexistentes: {(esc>=0).sum()}", file=sys.stderr)

EPS = [0, 1, 2, 3, 5, 7, 10]
t0 = time.perf_counter()
frente = frente_epsilon(P, esc, epsilons=EPS, time_limit=180, mip_gap=0.03)
dt = time.perf_counter() - t0
print(f"\nTiempo total barrido 10x10: {dt:.1f}s", file=sys.stderr)
...
with open("resultados/milp_10x10.json", "w") as fh:
    json.dump(out, fh, indent=2)
print("guardado en resultados/milp_10x10.json", file=sys.stderr)
